 ...
 */
function validateFileAccess(targetPath: string, operation: "read" | "write" | "delete", modeOverride?: unknown) {
 ...
 */
function validateShellCommand(command: string, modeOverride?: unknown) {
  const mode = getSecurityMode(modeOverride);
 ...
 */
function validateExternalAccess(url: string, modeOverride?: unknown) {
  const mode = getSecurityMode(modeOverride);

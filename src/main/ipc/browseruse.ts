...
function closeListeningServer(server: { close: (callback: () => void) => void } | null) {
...
async function restartServices(nextConfig?: Record<string, any>) {
...
async function sendToExtension(action: string, params: Record<string, any> = {}, timeout = config.actionTimeoutMs): Promise<any> {
...
async function cdp(tabId: unknown, method: string, params: Record<string, any> = {}, timeout?: number): Promise<any> {
...
function buildSnapshotScript({ limit, offset }) {
  ...
  function queryAll(root: any, css: string): any[] {
    try { return Array.from(root.querySelectorAll(css)); } catch (_) { return []; }
  }

  function selectorInRoot(el: any, root: any) {
    const parts = [];
    let node = el;
    while (node && node.nodeType === Node.ELEMENT_NODE && node !== root && parts.length < 6) {
      ...
      if (parent) {
        const siblings = Array.from(parent.children as HTMLCollection).filter((child) => child.tagName === node.tagName);
  ...
  function collect(root: any, context: Record<string, any>) {
...
async function screenshot(args) {
  ...
  const result: Record<string, any> = { path: filepath, filename };

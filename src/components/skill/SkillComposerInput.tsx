...
//   - 已选文件通过 onFilesChange(paths) 回传（按出现顺序、去重）
//
...
export interface SkillComposerHandle {
  ...
  insertFile: (file: { path: string; name: string }) => void;
  // 清空全部内容（文字、技能、文件）
  clear: () => void;
  // 聚焦编辑区
  focus: () => void;
...
function createFileChip(file: { path: string; name: string }): HTMLElement {
  const chip = document.createElement("span");
...
>(function SkillComposerInput(
  ...
  const emit = useCallback(() => {
    ...
    onFilesChange(extractAttrValues(root, FILE_ATTR));

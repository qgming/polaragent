...
import { deriveThinkingLabel } from "@/components/assistant-ui/elements/thinking-label";
...
/** 运行态标签（推导规则见 thinking-label.ts）。 */
const useThinkingLabel = (): string | undefined =>
  useAuiState((s) =>
    deriveThinkingLabel({
      running: s.message.status?.type === "running",
      parts: s.message.parts,
    }),
  );

/** 计时徽标：metadata.timing 只在流结束后才最终化，实时秒数必须自己走表。 */
const useElapsedLabel = (active: boolean): string | undefined => {
...
/** 助手消息内的实时状态行；无标签时不渲染任何内容。 */
const AssistantThinking: FC = () => {
  const label = useThinkingLabel();
  const elapsed = useElapsedLabel(label !== undefined);
  if (label === undefined) return null;
  return (
    // 外层消息内容区已带 px-2，这里只补上方间距，避免状态行比正文多缩进一层。
    <div data-slot="aui_assistant-thinking" className="pt-1">
        ...
        >
          {({ part, children }) => {
            switch (part.type) {
              ...
              case "indicator":
                // 运行态指示已由 AssistantThinking 统一承担（它还能汇报正在执行的
                // 工具名），这里不再重复渲染，避免出现两条状态行。
                return null;
              default:
                return null;
            }
...
const UserActionBar: FC = () => {
  return (
    ...
    >
      {/* PolarAgent：暂不支持用户消息就地编辑，避免触发 runtime throw */}
    </ActionBarPrimitive.Root>
  );
};

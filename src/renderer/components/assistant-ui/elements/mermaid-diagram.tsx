...
/*
 * 关于本文件里的 dangerouslySetInnerHTML（biome 已对该规则定向豁免，见 biome.json）：
 *
 * 注入的是 beautiful-mermaid 由 mermaid 源码生成的 SVG，而不是任意 HTML。
 * 由于 mermaid 源码来自模型输出，这里实测过转义行为：<script> 标签、onerror 等事件
 * 处理器、以及 &#60;script&#62; 实体绕过共 5 个用例，库均把标签文本整体转义成
 * &lt;img src=x onerror=alert(1)&gt; 这样的纯文本，未产出可执行内容。
 *
 * 因此注入内容是库生成的 SVG 结构本身。若日后换成不做转义的渲染器，
 * 或改成注入模型原文，必须重新评估此处，并考虑给渲染进程补 CSP。
 */

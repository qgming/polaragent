...
| 🔁 **GPT Image 2 资产交接** | [gptimage2-handoff.md](references/gptimage2-handoff.md)         | 先用 GPT Image 2 做角色/道具/场景/首帧，再交给 Seedance 图生视频 |
...
6. **GPT Image 2 前期资产优先**：当用户在做短剧、短片、电影、预告片，且还没有角色图、道具图、场景图或首帧图时，先参考 [gptimage2-handoff.md](references/gptimage2-handoff.md)，用 `gptimage2-prompt` 生成前期图片资产，再回到 Seedance 写视频提示词。

详见 [creative-strategy.md](references/creative-strategy.md)（创意策略）、[production-pipeline.md](references/production-pipeline.md)（长视频生产流水线）、[image-to-prompt.md](references/image-to-prompt.md)（图片驱动路径 C 完整方法论）、[gptimage2-handoff.md](references/gptimage2-handoff.md)（GPT Image 2 前期资产交接）。试片反馈后的**加长分段、清明脑洞、ASMR 真人向、AI 脱口秀**见 [11-用户反馈延展.md](projects/爆款实验/11-用户反馈延展.md)。
...
详细规范、prompt 模板和示例见 [image-generation.md](references/image-generation.md)。如果用户指定 GPT Image 2，或需要角色三视图、道具设定图、场景板、首帧/关键帧生产链路，先读取 [gptimage2-handoff.md](references/gptimage2-handoff.md)，把图片资产交给 `gptimage2-prompt` 生成，再用本技能组织 Seedance 视频提示词。

...
报告内容确认后，用内置的 `create_office_document` 工具生成PDF。流程如下：

### 1. 先撰写格式正确的HTML稿件

不要直接输出Markdown。将报告写作一份**自包含的完整HTML文档**，把所有CSS样式内嵌在 `<style>` 标签中。HTML中要包含封面页、章节结构、表格等完整内容。

### 2. HTML排版要求

使用以下CSS规则确保PDF渲染质量：

- **页面设置**：`@page { size: A4; margin: 25mm 20mm 20mm 20mm; }`
- **打印色彩**：`-webkit-print-color-adjust: exact; print-color-adjust: exact;`
- **分页控制**：封面后 `page-break-after: always`；大标题前 `page-break-before: always`；卡片/表格 `break-inside: avoid`
- **字体**：优先系统字体栈，`"Droid Sans Fallback", "Microsoft YaHei", Arial, sans-serif`
- **孤行控制**：`orphans: 3; widows: 3;`

### 3. 推荐的排版配色方案

供直接参考（与旧版WeasyPrint脚本保持一致）：

| 元素 | 样式 |
|------|------|
| 封面标题 | 28pt、#1a5276（深蓝）、居中、letter-spacing: 2pt |
| 封面副标题 | 14pt、#95a5a6（灰色）、居中 |
| H1（章节标题） | 20pt、#1a5276、下边栏2pt实线、分页前换页 |
| H2（子章节） | 14pt、#1e8449（绿色） |
| H3 | 12pt、#2e86c1（浅蓝） |
| H4 | 11pt、#5b2c6f（紫色） |
| 正文 | 10.5pt、#2c3e50、行距1.75、两端对齐 |
| 引用块 | 左侧3pt #1a5276竖线 + 浅灰背景 |
| 表格 | 全宽、深蓝表头白字、斑马纹交替行 |
| 页眉 | 标题 + " | 横纵分析法深度研究报告" |
| 页脚 | "第 X 页" |
...
<head>
  ...
  <style>
    ...
    @page {
      ...
      @top-center {
        content: "报告标题 | 横纵分析法深度研究报告";
        font-family: "Droid Sans Fallback", "Microsoft YaHei", Arial, sans-serif;
        font-size: 8pt;
        color: #95a5a6;
    ...
    body {
      font-family: "Droid Sans Fallback", "Microsoft YaHei", Arial, sans-serif;
      font-size: 10.5pt; line-height: 1.75; color: #2c3e50;
...
<body>
  <!-- 封面页 -->
  <div class="cover" style="page-break-after: always; text-align: center; padding-top: 45%;">
    <h1 style="font-size: 28pt; color: #1a5276; margin-bottom: 8mm; letter-spacing: 2pt;">研究对象名称</h1>
    <div style="font-size: 14pt; color: #95a5a6; margin-bottom: 6mm;">横纵分析法深度研究报告</div>
    <hr style="width: 60%; border-top: 1.5pt solid #1a5276; margin: 8mm auto;">
    <div style="font-size: 11pt; color: #95a5a6;">作者: 数字生命卡兹克</div>
  </div>
...
### 5. 调用 create_office_document 工具

HTML稿件写完后，调用内置工具生成PDF：
...
```json
{
  ...
  "pdfStyle": "whitepaper",
  "path": "output/研究对象名称_横纵分析报告.pdf"
}
```

**注意**：传 `html` 参数时，一定要传完整的HTML文档（以 `<!doctype html>` 或 `<html>` 开头）。内置工具会自动保留你写的全部CSS样式，不会覆盖。

**风格建议**：研究报告推荐使用 `whitepaper`（白皮书）或 `academic`（学术论文）或 `classic`（古典优雅）风格。如需更现代的排版可选 `modern`。内置工具共支持 14 种风格，详见 `pdf-generator` 技能文档。

### 6. 生成后的验证

PDF生成后检查：
- [ ] 封面页渲染正常（标题、副标题、作者、分隔线）
- [ ] 页眉页脚正确显示（首页不显示）
- [ ] 中文字体渲染无缺失（所有字符正常显示为方块字）
- [ ] 表格宽度适配页面，斑马纹行可见
- [ ] 引用块左侧竖线可见
- [ ] 标题层级颜色正确（深蓝/绿色/浅蓝/紫色）
- [ ] 没有文字重叠或溢出的问题
...
- [ ] 所有关键事实标注了信息来源？
- [ ] 搜不到的信息诚实标注了「暂缺」，没有编造？
- [ ] HTML稿件是完整的（`<!doctype html>` + `<style>` + 封面 + 正文）？
- [ ] 使用 `create_office_document` 生成的PDF，没有依赖外部脚本？
- [ ] PDF排版美观、结构清晰、可读性好？

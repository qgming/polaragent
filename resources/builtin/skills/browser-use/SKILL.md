...
- 🔍 **网络监控** - 监听 HTTP 请求和响应
- 🧩 **组件穿透** - 快照、点击、填充和元素截图支持同源 iframe 与开放 Shadow DOM
...
**重要提示**：
- @e 引用仅在当前 snapshotId 内稳定，后续 click/fill/screenshot 可传入 snapshotId
- 页面内容变化后需重新调用 snapshot
- 建议每次点击/填充前先生成快照
- 对嵌入在同源 iframe 或开放 Shadow DOM 里的控件，优先使用 snapshot 返回的 @e 引用
...
- `snapshotId` (可选) - 使用 @e 引用时建议传入 browser_snapshot 返回的 snapshotId
...
// 使用 @e 引用（推荐）
const snap = browser_snapshot({ tabId: 123 })  // 先生成快照
browser_click({ tabId: 123, target: "@e1", snapshotId: snap.snapshotId })
```
...
### 11. browser_console - 控制台日志

**功能**：监听页面 console 日志与异常。
...
browser_click({ tabId: 123, target: "button.refresh" })
